/*
  # Comprehensive QuizWhiz Platform Updates

  1. Enhanced Tables
    - Add real-time notifications for students
    - Add leaderboard tracking
    - Add weekly performance monitoring
    - Add download tracking
    - Enhanced group quiz management
  ...
  3. Security
    ...
    - Proper policies for live updates
*/

-- Notifications table for real-time student updates
...
-- Weekly performance tracking for students
...
ALTER TABLE group_quizzes ADD COLUMN IF NOT EXISTS allow_retakes boolean DEFAULT false;
...
-- Function to notify all students when group quiz is created
...
BEGIN
  ...
  SELECT 
    ...
    'Teacher ' || (SELECT username FROM users WHERE id = NEW.teacher_id) || ' created "' || NEW.title || '". Code: ' || NEW.quiz_code,
    jsonb_build_object(
...
-- Function to update weekly performance daily
CREATE OR REPLACE FUNCTION update_weekly_performance()
RETURNS TRIGGER AS $$
DECLARE
  week_start date;
  week_end date;
  prev_week_avg decimal;
BEGIN
  -- Calculate current week boundaries (Monday to Sunday)
  week_start := date_trunc('week', CURRENT_DATE)::date;
  week_end := (week_start + interval '6 days')::date;
  
  -- Get previous week average for improvement calculation
  SELECT COALESCE(average_score, 0) INTO prev_week_avg
  FROM weekly_performance 
  WHERE user_id = NEW.user_id 
    AND week_start = (date_trunc('week', CURRENT_DATE)::date - interval '7 days')::date;
  
  -- Update or insert weekly performance
  INSERT INTO weekly_performance (user_id, week_start, week_end, total_quizzes, total_score, average_score, best_score, improvement_percentage)
  VALUES (
    NEW.user_id,
    week_start,
    week_end,
    ...
    NEW.percentage,
    NEW.score,
    CASE WHEN prev_week_avg > 0 THEN ((NEW.percentage - prev_week_avg) / prev_week_avg * 100) ELSE 0 END
  )
  ...
  DO UPDATE SET
    ...
    improvement_percentage = CASE 
      WHEN prev_week_avg > 0 THEN 
        (((weekly_performance.total_score + NEW.score) / (weekly_performance.total_quizzes + 1) - prev_week_avg) / prev_week_avg * 100)
      ELSE 0 
    END,
    updated_at = now();
  
  -- Update user's current week score and streak
  UPDATE users 
  SET 
    current_week_score = (
    ...
    streak_days = CASE 
      WHEN DATE(last_login) = CURRENT_DATE - 1 THEN streak_days + 1
      WHEN DATE(last_login) = CURRENT_DATE THEN streak_days
      ELSE 1
    END,
    last_login = now(),
    updated_at = now()
...
-- Function to calculate weekly rankings (run daily)
...
CREATE INDEX IF NOT EXISTS idx_weekly_performance_user_week ON weekly_performance(user_id, week_start);
CREATE INDEX IF NOT EXISTS idx_leaderboards_group_quiz ON leaderboards(group_quiz_id, rank_position);
CREATE INDEX IF NOT EXISTS idx_quiz_downloads_user_id ON quiz_downloads(user_id, created_at DESC);
