...
import { Trophy, Medal, Award, Clock, Target, TrendingUp, Users, Crown, Star } from 'lucide-react';
...
export const LiveLeaderboard: React.FC<LiveLeaderboardProps> = ({
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          loadLeaderboard(); // Reload the entire leaderboard for real-time updates
        }
      )
      .subscribe();
  ...
  const getPerformanceBadge = (percentage: number) => {
    if (percentage >= 95) return { text: 'Perfect!', color: 'bg-green-100 text-green-800' };
    if (percentage >= 90) return { text: 'Excellent', color: 'bg-green-100 text-green-700' };
    if (percentage >= 80) return { text: 'Great', color: 'bg-blue-100 text-blue-700' };
    if (percentage >= 70) return { text: 'Good', color: 'bg-yellow-100 text-yellow-700' };
    if (percentage >= 60) return { text: 'Fair', color: 'bg-orange-100 text-orange-700' };
          ...
          <div className="space-y-3">
            <AnimatePresence>
              {leaderboard.map((entry, index) => {
                const badge = getPerformanceBadge(entry.percentage);
                return (
                  <motion.div
                    key={entry.id}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between p-4 pl-6 relative">
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500">High Scorers (80%+)</div>
            </div>
          </div>
        </div>
      )}
