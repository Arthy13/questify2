...
import { Calendar, TrendingUp, TrendingDown, Award, Target, Clock, BarChart3, Users } from 'lucide-react';
...
export const WeeklyPerformance: React.FC<WeeklyPerformanceProps> = ({
  ...
  const getPerformanceLevel = (score: number) => {
    ...
    if (score >= 80) return { level: 'Great', color: 'text-blue-600', bg: 'bg-blue-100' };
        ...
        <div className="flex items-center justify-between">
          ...
          {currentWeek && (
            <div className="flex items-center space-x-2">
              ...
              {currentWeek.average_score > 0 && (
                <div className={`px-3 py-1 rounded-full text-sm font-medium ${getPerformanceLevel(currentWeek.average_score).bg} ${getPerformanceLevel(currentWeek.average_score).color}`}>
                  {getPerformanceLevel(currentWeek.average_score).level}
                </div>
              )}
            </div>
          )}
      ...
      <div className="p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center p-3 bg-orange-50 rounded-lg">
                <div className={`text-2xl font-bold flex items-center justify-center space-x-1 ${getImprovementColor(currentWeek.improvement_percentage)}`}>
                  ...
                  <span>{Math.abs(Math.round(currentWeek.improvement_percentage))}%</span>
                </div>
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center space-x-3 text-sm text-gray-600">
                      <span>{week.total_quizzes} quizzes</span>
                      <span>•</span>
                      <div className="flex items-center space-x-1">
                        <Users className="h-3 w-3" />
                        <span>Rank #{week.rank_position}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className={`text-sm flex items-center space-x-1 ${getImprovementColor(week.improvement_percentage)}`}>
                      ...
                      <span>{Math.abs(Math.round(week.improvement_percentage))}%</span>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        {weeklyData.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-400">Complete some group quizzes to see your weekly performance</p>
          </div>
        )}
      </div>
    </div>
