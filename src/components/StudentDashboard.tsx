...
+                  {notification.data?.quiz_code && (
+                    <p className="text-xs text-blue-600 font-mono">
+                      Code: {notification.data.quiz_code}
+                    </p>
+                  )}
+                </div>
...
+                    <button
+                      onClick={() => {
+                        setQuizCode(notification.data.quiz_code);
+                        handleJoinQuiz();
...
+              <p className="text-blue-100">Upload materials or paste text to generate quizzes</p>
             </div>
           </div>
         </motion.button>
...
+            {savedQuizzes.slice(0, 6).map((quiz, index) => (
+              <motion.div
+                key={quiz.id}
+                initial={{ opacity: 0, y: 10 }}
+                animate={{ opacity: 1, y: 0 }}
...
+                  <p className="font-medium text-gray-900">{quiz.title}</p>
+                  <p className="text-sm text-gray-500">
+                    {quiz.questions.length} questions • {quiz.type.toUpperCase()}
+                  </p>
+                </div>
+                <div className="flex items-center justify-between">
